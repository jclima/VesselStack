...
for _attempt in $(seq 1 60); do
    if curl -fsS http://127.0.0.1:8086/health >/dev/null && \
        curl -fsS http://127.0.0.1:43000/api/health >/dev/null; then
        ready=1
        break
    fi
    sleep 2
...
curl -fsS http://127.0.0.1:43000/api/health >/dev/null
...
docker exec vesselstack-influxdb influx bucket list \
    --org vesselstack --token integration-token-0123456789 --name signalk_1m \
    --hide-headers | grep -q signalk_1m
docker exec vesselstack-influxdb influx task list \
...
curl -fsS -u boatadmin:integration-grafana \
    http://127.0.0.1:43000/api/datasources/uid/vesselstack-influx | \
    jq -e '.name == "VesselStack InfluxDB"' >/dev/null
curl -fsS -u boatadmin:integration-grafana \
    http://127.0.0.1:43000/api/dashboards/uid/vesselstack-system-health | \
