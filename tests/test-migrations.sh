...
"$ROOT/scripts/migrate.sh" "$TEST_ROOT/config/vesselstack.env" 1.1.0 >/dev/null
grep -qx '1.1.0' "$TEST_ROOT/config/installed-version"
...
printf '9.0.0\n' > "$TEST_ROOT/config/installed-version"
if "$ROOT/scripts/migrate.sh" "$TEST_ROOT/config/vesselstack.env" 1.1.0 2>/dev/null; then
